use hypernote_mdx::ast::{
    Ast, FrontmatterFormat, Node, NodeData, NodeTag, Range, SourcePosition, TableAlignment,
};
...
fn ast_accessors_return_safe_defaults_for_invalid_indices() {
    ...
    assert_eq!(SourcePosition { line: 1, column: 3 }, ast.line_col(1_000));
    assert_eq!(
        SourcePosition { line: 1, column: 1 },
        ast.node_position(999)
    );
    ...
    assert!(ast.jsx_element_view(999).is_none());
...
fn malformed_ast_defaults_are_consistent() {
    ...
    let attrs = ast
        .jsx_attribute_views(3)
        .expect("JSX node should produce typed attrs");
    assert!(attrs.is_empty());
    let jsx = ast
        .jsx_element_view(3)
        .expect("JSX node should produce typed element view");
    assert_eq!("", jsx.name);
    assert!(jsx.attrs.is_empty());
